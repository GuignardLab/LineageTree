...
- Common: These properties are usually expected from a dataset and if they exist they will be taken into account for byt the object.
    1. **time**: A dictionary mapping each node to a timepoint.The timepoints must respect the lineage hierarchy, meaning if `n1` resides in `t` its successor `n2` resides in `t+1`. Alternatively the user may set a starting time using starting time= t

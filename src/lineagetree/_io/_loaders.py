"""Loaders that read lineage trees from the file formats of tracking tools.

Each ``read_from_*`` function returns a
[`LineageTree`][lineagetree.LineageTree]. Node ids are renumbered by most
loaders; the ids of the file, when kept, are stored in a property.

Several text formats are parsed with ``eval``: only read files from sources
you trust.
"""
...
def read_from_bmf(
    ...
    """Read a lineage tree from a binary mesh format (BMF) file.

    Each track of the file becomes one chain, without divisions. The
    position of a node is the centre of mass of its mesh.
    ...
    store_meshes : bool, default=True
        Whether to store the meshes in the ``mesh`` property, which maps
        each node to a dictionary with the keys ``"vertices"``, ``"faces"``
        and ``"center_mass"``.
    ...
    translation : tuple of float, default=(0.0, 0.0, 0.0)
        Translation for the x, y, z coordinates, applied after the
        multipliers.
    name : str, optional
        Name of the lineage tree. Defaults to the file name without its
        extension.
...
def read_from_csv(
    ...
    The file has no header and one row per node. The column order depends on
    ``link``:

    - ``link=1``: ``id, time, z, y, x, pred_id, lin_id``;
    - ``link=2``: ``time, z, y, x, id, pred_id, lin_id``;
    - any other value: ``id, time, z, y, x``, without edges.

    ``pred_id`` is the id of the predecessor. Node ids are renumbered.
    ...
    z_mult : float, default=1
        Multiplier applied to z, to correct the anisotropy of the images.
    link : int, default=1
        The column layout, see above.
    delim : str, default=","
...
def read_from_ASTEC(
    ...
    ----------
    file_path : str
        Path to an ASTEC output, ``.xml`` or ``.pkl``.
    eigen : bool, default=False
        Not used.
    name : str, optional
        Name of the lineage tree. Defaults to the file name without its
...
def read_from_binary(fname: str, name: None | str = None) -> LineageTree:
    """Read a lineage tree from the legacy binary format.

    The file holds three int64 lengths, then the tree as an int64 sequence
    (node ids, with ``-1`` and ``-2`` as chain and division markers), the
    time of each root as uint16, and the 3D positions as float64.
...
def read_from_txt_for_celegans(
    ...
    """Read a C. elegans lineage from a tab-separated text file.

    The first line is a header. Each other line is one cell at one time
    point: ``time, cell name, x, y, z``. A cell is linked to the cell of the
    previous time point with the same name, with its name minus the last
    letter (a division), or with its known mother for the founder cells (AB,
    P1, EMS, …). The cell names become the labels.
...
def read_from_txt_for_celegans_CAO(
    ...
    """Read a C. elegans lineage from the format of Cao et al.

    The first line is a header. Each other line is one cell at one time
    point, whitespace-separated: ``cell name, time, z, x, y``. Cells are
    linked as in
    [`read_from_txt_for_celegans`][lineagetree.read_from_txt_for_celegans].
    The cell names are stored in the ``label`` property.
    ...
    raw_size : np.ndarray, optional
        Shape of the raw image volume, used to flip the z-axis when
        ``reorder=True``.
...
def read_from_txt_for_celegans_BAO(
    ...
    """Read a C. elegans expression file from Bao et al.

    Files are available at http://digital-development.org. Each line is one
    cell, tab-separated: the cell name first, and the expression values of
    each of its time points, comma-separated, last. The file gives no
    absolute times: each root starts at time 0. The values are stored in
    the ``expression`` property and the cell names become the labels.
...
def read_from_tgmm_xml(
    ...
    """Read a lineage tree from the XML outputs of TGMM.

    TGMM writes one XML file per time point. Only the cells with a valid
    position and Gaussian parameters (``alpha``, ``W``, ``nu``,
    ``alphaPrior``) are kept. The properties ``svIdx``, ``lin``, ``C_lin``,
    ``coeffs`` and ``intensity`` are read from the files.

    Parameters
    ----------
    file_format : str
        Path pattern of the XML files, formatted with ``t``, e.g.
        ``path/to/xml/standard_name_t{t:06d}.xml`` for times written with 6
        digits, zero-padded.
    tb : int
        First time point to read.
    te : int
        Last time point to read (included).
...
def read_from_mastodon(
    ...
    """Read a lineage tree from a Mastodon project.

    Each tag set of the project becomes a property of the tree.
    ...
    tag_set : str, optional
        Name of the tag set used as labels. If not given or not found, the
        first tag set is used.
    name : str, optional
        Name of the lineage tree. Defaults to the file name without its
        extension.
...
def read_from_mastodon_csv(
    ...
    """Read a lineage tree from the CSV export of Mastodon.

    Node ids are the Mastodon spot ids, also stored as strings in the
    ``label`` property.

    Parameters
    ----------
    paths : list of str
        Paths to the spots CSV file and to the links CSV file, in that
        order.
    name : str, optional
        Name of the lineage tree. Defaults to the file name without its
        extension.
...
def read_from_mamut_xml(
    ...
    """Read a lineage tree from a MaMuT (or TrackMate) XML file.

    Node ids are the spot ids of the file. The spot names are stored in the
    ``label`` property, the tracks in ``tracks`` and ``track_name``, and
    the ``TISSUE_NAME`` and ``TISSUE_TYPE`` attributes, when present, in
    ``fate`` and ``fate_nb``.
    ...
    xml_attributes : list of str, optional
        Names of additional spot attributes to read and store as node
        properties.
    name : str, optional
        Name of the lineage tree. Defaults to the file name without its
        extension.
...
def read_from_swc(swc_path: Path | str) -> LineageTree:
    ...
    ``LineageTree`` is created with ``temporal=False`` because SWC files do
    not encode time; the time of a node is its depth. Node ids are the
    sample ids of the file.
...
"""Loaders by file extension (lower case), then by description."""

            ...
            while i < len(lines):
                if not lines[i].strip():
                    ...
                    if next_i < len(lines):
                        ...
                        if (next_line and 
                            next_i + 1 < len(lines) and 
                            set(lines[next_i + 1].strip()).issubset({"-", "="}) and 
                            lines[next_i + 1].strip()):
                ...
                if lines[i].strip() and ":" in lines[i]:
                    ...
                    while i < len(lines):
                        if not lines[i].strip():
                            ...
                            i += 1
                        elif len(lines[i]) - len(lines[i].lstrip()) > current_indent:
...
def attach_methods(cls, funcs):
    ...
    if hasattr(funcs, "__dict__"):
        items = {name: obj for name, obj in funcs.__dict__.items() if callable(obj) and not name.startswith("_")}
...
def _should(name, obj, cls_module):
    ...
    try:
        ...
        return False
    return first.kind in (inspect.Parameter.POSITIONAL_ONLY,
...
class AutoMethodizeMeta(type):
    def __new__(mcls, name, bases, ns, **kw):
        ...
        if "__methodize__" in ns:
            ...
            funcs = {n: getattr(cls, n) for n, v in ns.items() if _should(n, v, ns.get("__module__"))}
